...
When the Home Forecaster publishes a manual import-price override strictly below the regular overnight off-peak rate, the controller can temporarily preserve battery energy for later use. During a qualifying active interval outside the regular overnight window, it uses PauseDischarge when no additional battery charge is justified by the available forecast. Where the forecast demonstrates a shortfall before the next regular cheap period, it can temporarily use the charge slot for a bounded top-up. A temporary override does not change the inverter's user-owned charge-target SoC. Power Down, Axle, calibration and confirmed EV Smart Charging retain priority. If the forecast or live battery inputs are missing, the controller retains its existing charging plan. Future override windows are currently diagnostic only: the controller does **not yet** reduce the preceding regular overnight charge in anticipation of them.
