...
## Explicit cheaper import-price overrides (experimental)

When the Home Forecaster publishes a manual import-price override strictly below the regular overnight off-peak rate, the controller can temporarily preserve battery energy for later use. During a qualifying active interval outside the regular overnight window, it uses PauseDischarge when no additional battery charge is justified by the available forecast. Where the forecast demonstrates a shortfall before the next regular cheap period, it can temporarily use the charge slot for a bounded top-up. A temporary override does not change the inverter's user-owned charge-target SoC. Power Down, Axle, calibration and confirmed EV Smart Charging retain priority. If the forecast or live battery inputs are missing, the controller retains its existing charging plan. When a cheaper window follows the regular overnight period, the controller can reduce a not-yet-active overnight charge if the intervening no-slots forecast is complete, the safety buffer is covered, and the cheaper window has enough charging capacity. If these checks fail, the normal overnight objective remains in force.
