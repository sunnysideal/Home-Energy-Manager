...
def _event_from_state(controller, state):
    ...
    if not available or event_type != 'export' or not start or not end or end <= start:
        return None
    return {'start': start.astimezone(controller.tz), 'end': end.astimezone(controller.tz), 'event_type': event_type}
...
def _disabled_discharge(controller, plan, anchor):
    ...
    plan['discharge'] = {
        ...
        'planned_kwh': 0.0, 'kind': 'axle_protection',
    }
...
def _minimise_calibration_discharge(controller, plan, window, capacity, reserve, max_discharge, forecast):
    ...
    plan['discharge'] = {
        'start': core.iso(discharge_start), 'end': core.iso(expected_floor.replace(second=0, microsecond=0)),
        'rate_w': int(round(max_discharge)), 'target_soc': int(round(reserve)),
        'planned_kwh': round(discharge_kwh, 3), 'kind': 'calibration_to_reserve',
    }
...
async def _apply_axle_overlay(controller, forecast, plan, window, capacity, reserve, max_charge, max_discharge):
    ...
    required_soc = min(100.0, raw_required_soc); full_event_possible = raw_required_soc <= 100.0
    diagnostics = {
        'entity': axle_entity, 'event_type': 'export', 'event_start': core.iso(start), 'event_end': core.iso(end),
        'required_event_start_soc': round(required_soc, 1), 'full_event_possible': bool(full_event_possible),
        'max_discharge_rate_w': int(round(max_discharge)), 'action': 'protecting',
    }

    intelligent = await controller.intelligent_go_info(); confirmed_intelligent = bool(intelligent.get('confirmed'))
    if start <= now < end:
        if confirmed_intelligent:
            _disabled_discharge(controller, plan, now); diagnostics['action'] = 'suspended_for_confirmed_ev_smart_charging'
        else:
            plan['charge'] = dict(plan.get('charge', {})); plan['charge']['start'] = core.iso(now.replace(second=0, microsecond=0)); plan['charge']['end'] = plan['charge']['start']; plan['charge']['planned_kwh'] = 0.0
    ...
    if cheap_before_event:
        ...
        if complete:
            bridge_stored_kwh = max(0.0, net_kwh) / discharge_eff; cheap_target = min(100.0, required_soc + bridge_stored_kwh / float(capacity) * 100.0)
        else:
            cheap_target = 100.0
    ...
    if forecast_event_soc is None:
        forecast_event_soc = live_soc; coverage_complete = False
    else:
        coverage_complete = True
    if forecast_event_soc is None:
        forecast_event_soc = float(reserve); coverage_complete = False

    shortfall_pct = max(0.0, required_soc - float(forecast_event_soc)); shortfall_kwh = float(capacity) * shortfall_pct / 100.0
    stored_kw = max(0.1, (float(max_charge) / 1000.0) * charge_eff); charge_hours = shortfall_kwh / stored_kw
    margin = max(0, int(controller.c.get('axle_charge_margin_minutes', 10))); latest_charge_start = start - timedelta(hours=charge_hours, minutes=margin)
    diagnostics.update({'forecast_event_start_soc': round(float(forecast_event_soc), 1), 'forecast_coverage_complete': bool(coverage_complete), 'shortfall_kwh': round(shortfall_kwh, 3), 'latest_charge_start': core.iso(latest_charge_start)})
...
async def _plan_with_policies(self, forecast, soc, window, fallback=False):
    ...
    if not fallback and self.operation_mode() == 'minimise_export':
        ...
        if capacity is not None and reserve is not None and capacity > 0:
            ...
            if active:
                ...
                bridge_start = window['end']; next_offpeak_start = _shift_local_day(self, window['start'], 1)
            protected_soc, required_peak_kwh, complete = self.power_down_protected_soc(forecast, bridge_start, next_offpeak_start, capacity, reserve)
            ...
            if self.calibration_state() == 'deep_recharge' and max_charge is not None and max_charge > 0:
                self._calibration_max_charge_w = float(max_charge); self.c['max_charge_c_rate'] = max(float(original_max_c_rate), float(max_charge) / (float(capacity) * 1000.0))
    ...
    try:
        ...
        self.c['minimise_export_min_soc'] = original_floor; self.c['max_charge_c_rate'] = original_max_c_rate
    ...
    if not fallback and self.operation_mode() in ('minimise_export', 'maximise_export', 'export_generated'):
        ...
        if reserve is None: reserve, _ = await self.num('battery_reserve_entity', 'battery_reserve', True)
        if max_charge is None: max_charge, _ = await self.num('inverter_max_charge_rate_entity', 'max_charge_rate', False)
        if max_discharge is None: max_discharge, _ = await self.num('inverter_max_discharge_rate_entity', 'max_discharge_rate', False)
        if None not in (capacity, reserve, max_charge, max_discharge) and capacity > 0 and max_charge > 0 and max_discharge > 0:
            result = await _apply_axle_overlay(self, forecast, result, window, float(capacity), float(reserve), float(max_charge), float(max_discharge))
    return result
