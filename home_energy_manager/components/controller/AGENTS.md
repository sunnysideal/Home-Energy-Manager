...
7. **Regular overnight target**
   ...
   - **Approved manual-price exception (issue #107):** a verified, explicit manual import-price override strictly cheaper than the normal regular overnight price may defer *only safely deferrable* regular charging beyond the overnight window. This applies to `maximise_export` and `export_generated` as an exception to their normal 100% overnight objective. The controller must demonstrate complete forecast coverage, adequate stored energy for all intervening peak-rate demand and the configured safety buffer, and sufficient confirmed cheap-window charge capacity before reducing the regular charge. Incomplete or uncertain evidence retains the normal overnight objective. Calibration, Power Down, Axle, and confirmed EV Smart Charging priorities remain intact; a future unconfirmed EV dispatch never qualifies.
