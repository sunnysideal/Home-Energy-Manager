...
from controller_battery import (
    soc_at as _soc_at,
    forecast_battery_kwh_between as _forecast_battery_kwh_between,
    ...
    choose_rate_and_start as _choose_rate_and_start,
    band_factor as _band_factor,
    dwell as _dwell,
    charge_minutes as _charge_minutes,
    choose_rate as _choose_rate,
)
...
class Controller(_legacy.Controller):
    ...
    def soc_at(self, state, when, attr='forecast_no_slots'):
        return _soc_at(self, state, when, attr)

    def forecast_battery_kwh_between(self, state, start, end, attr='forecast_no_slots'):
        return _forecast_battery_kwh_between(self, state, start, end, attr)

    def planned_discharge_soc_adjustment(self, state, ds, de, rate_w, cap, reserve):
        return _planned_discharge_soc_adjustment(self, state, ds, de, rate_w, cap, reserve)

    def projected_charge_start_soc(self, state, when, forced_adjustment_pct, reserve):
        return _projected_charge_start_soc(self, state, when, forced_adjustment_pct, reserve)

    def latest_charge_start_for_rate(self, state, window, target, rate, cap, reserve, forced_adjustment_pct, earliest=None):
        return _latest_charge_start_for_rate(
            self, state, window, target, rate, cap, reserve, forced_adjustment_pct, earliest
        )

    def choose_rate_and_start(self, state, window, target, cap, reserve, forced_adjustment_pct, hw, earliest=None):
        return _choose_rate_and_start(
            self, state, window, target, cap, reserve, forced_adjustment_pct, hw, earliest
        )

    def band_factor(self, band):
        return _band_factor(self, band)

    def dwell(self):
        return _dwell(self)

    def charge_minutes(self, soc, target, rate, cap):
        return _charge_minutes(self, soc, target, rate, cap)

    def choose_rate(self, soc, target, cap, window, hw):
        return _choose_rate(self, soc, target, cap, window, hw)
