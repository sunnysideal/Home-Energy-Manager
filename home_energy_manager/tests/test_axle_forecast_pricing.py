...
def test_home_forecaster_runs_through_axle_pricing_wrapper():
    assert '/app/runtime/home_forecaster/axle_pricing_runner.py' in LAUNCHER.read_text()
    assert 'COPY components/home_forecaster/app/axle_pricing_runner.py /app/runtime/home_forecaster/axle_pricing_runner.py' in DOCKERFILE.read_text()
