...
def _sqlite_integrity(path: Path) -> None:
    ...
    if not row or row[0] != "ok":
        raise RuntimeError(
            f"SQLite integrity check failed for {path.name}: {row[0] if row else 'no result'}"
        )
...
def export_migration_bundle(
    ...
    """Export learned state plus add-on options using migration format v2."""
    ...
    with tempfile.TemporaryDirectory(prefix="hem-migration-export-") as tmp_name:
        ...
        manifest = {
            "format_version": 2,
            "home_energy_manager_version": "0.1.40",
            "files": files,
        }
        (tmp / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
        if options_path.exists():
            shutil.copy2(options_path, tmp / "options.json")
        ...
        with zipfile.ZipFile(temp_bundle, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            for item in files:
                archive.write(tmp / item["name"], item["name"])
            if (tmp / "options.json").exists():
                archive.write(tmp / "options.json", "options.json")
...
def import_migration_bundle(
    ...
    if existing and not marker_exists:
        raise RuntimeError(
            "Refusing migration import because learned-state files already exist without a migration marker: "
            + ", ".join(existing)
        )
    ...
    with tempfile.TemporaryDirectory(prefix="hem-migration-import-") as tmp_name:
        ...
        if isinstance(migrated_options, dict):
            _restore_supervisor_options(migrated_options)
...
def _normalise_home_forecaster(hf: dict) -> None:
    """Compatibility wrapper retained for callers/tests from the 0.1.x launcher."""
    wrapper = {
        "home_forecaster": hf,
        "ashp_forecaster": {},
        "controller": {},
    ...
    runtime, _, _ = migrate_runtime_options(wrapper)
    hf.clear()
    hf.update(runtime["home_forecaster"])
...
def _resolve_home_assistant_timezone(raw):
    ...
    print(f"[manager] runtime timezone={timezone_name} (Home Assistant)", flush=True)
    return timezone_name


def load_and_split_options():
    saved = json.loads(OPTIONS.read_text())
    if handle_migration(saved) == "stop":
        return None
    ...
    os.environ["HOME_ENERGY_MANAGER_VERSION"] = "0.1.40"
