...
def test_weather_extension_reaches_beyond_configured_48_hour_horizon(monkeypatch):
    ...
    raw = [{"datetime": (now + timedelta(hours=i)).isoformat(), "temperature": 11.0 + i * 0.1} for i in range(45)]
    client = object.__new__(runner.HorizonHAClient); client.forecast_hours = 48; client.tz = tz
...
def test_weather_is_not_extended_when_provider_already_covers_horizon(monkeypatch):
    tz = ZoneInfo("Europe/London")
    now = datetime.now(tz).replace(minute=0, second=0, microsecond=0)
    raw = [{"datetime": (now + timedelta(hours=i)).isoformat(), "temperature": 10.0} for i in range(60)]
    client = object.__new__(runner.HorizonHAClient); client.forecast_hours = 48; client.tz = tz
    monkeypatch.setattr(legacy.HAClient, "get_hourly_weather", lambda self, entity_id: list(raw))
...
def test_dhw_selector_waits_for_fresh_thermal_horizon(monkeypatch):
    calls = []
    thermal_values = [0.0] * 96; thermal_values[10] = 1.25
    ...
    cfg = SimpleNamespace(update_minutes=15); client = SimpleNamespace(get_state=lambda entity_id: {}); store = SimpleNamespace(db=object())
    starts = [datetime.now(ZoneInfo("Europe/London")) + timedelta(minutes=30 * i) for i in range(96)]
    result = runner._select_dhw_with_refresh_wait(client, store, cfg, starts)
    assert len(calls) == 2
    assert result.source == "thermal" and result.reason == "thermal_authoritative" and result.values == thermal_values


def test_dhw_selector_fails_when_thermal_remains_unavailable(monkeypatch):
    calls = []
    def fake_select(*args, **kwargs):
        calls.append(1)
    ...
    with pytest.raises(RuntimeError, match="thermal production forecast unavailable"):
        runner._select_dhw_with_refresh_wait(client, store, cfg, starts)
    assert len(calls) >= 1
